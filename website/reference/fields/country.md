...
> 🏷️ 字段类别：**国家** · 🔑 JSON Key：`country` · 🧩 Go 字段：`IPInfo.Country`

::: tip 🎨 一图抵千言
`country` 在 `IPInfo` 结构体中所属分组与相关字段关系如下：
...
`country` 归属于 **geo（地理）** 分组，与 [`country_code`](./country_code) 取值一致，并与 [`country_name`](./country_name)、[`country_code_iso3`](./country_code_iso3) 等共同构成国家维度的字段族。
:::
...
🎯 `country` 表示目标 IP 地址所属国家的 **ISO 3166-1 alpha-2（2 位字母）国家代码**。
...
::: tip 💡 与 `country_code` 等价
`country` 与同结构体内的 [`country_code`](./country_code) 在 ipapi.co 的响应中**取值完全一致**，两者都返回 ISO 3166-1 alpha-2 代码。区别仅在 JSON key 名不同：`country` 是早期即沿用的简短键名，`country_code` 是语义更明确的命名键。二者可互换使用，任选其一即可。
:::
...
`Country` 本身是普通的 `string` 类型，**不是** 指针字段，因此可以直接访问，无需判空。
...
对于带有 `omitempty` 的字段（如 `Hostname string \`json:"hostname,omitempty"\``），当 API 返回中不包含该字段时，Go 端会保留零值（空字符串），调用方需自行处理空值语义。而 `country` 既非指针也非 `omitempty`，访问最为直接。

> 📝 注意：`Country` 字段本身没有专属的 `Get` 访问器方法（不像 `Postal` 有 `GetPostal()`），因为它不是指针字段，直接读取 `info.Country` 即可，无需额外封装。
...
func main() {
    ...
    // 直接访问 Country 字段
    fmt.Println("国家代码:", info.Country) // 输出: US
}
```

### 2️⃣ GetField 单字段查询

只需查询 `country` 这一个字段时，使用 `GetField` 更轻量，仅发起一次单字段请求：

```go
code, err := client.GetField(ctx, "8.8.8.8", "country")
if err != nil {
    log.Fatal(err)
}
...
查询**当前发起请求的客户端 IP** 的 `country`（不传 IP 参数，对应 `GET /{field}/` 端点）：

```go
code, err := client.GetClientField(ctx, "country")
if err != nil {
    log.Fatal(err)
}
...
- 🏳️ **与同义字段互换**：与 [`country_code`](./country_code) 等价，可按团队命名习惯任选其一。
...
- 📖 阅读 [字段总览](../../api/fields)，了解 `country` 在整个字段体系中的位置。
- 🏳️ 进入 [国家类字段分类页](../../api/field-country)，对比 `country` 与 `country_name`、`country_code`、`country_code_iso3` 的差异。
...
| --- | --- |
| JSON Key | `country` |
| Go 字段 | `IPInfo.Country` |
| Go 类型 | `string` |
| 分组 | geo（地理） |
| 示例值 | `US` |
| 是否指针字段 | 否 |
| 是否 omitempty | 否 |
| 同义字段 | [`country_code`](./country_code) |
:::
